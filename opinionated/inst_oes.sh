...
kubectl create secret docker-registry oes-repo --docker-username=opsmxuser --docker-password=OpsMx@321 --docker-email=opsmx@example.com --namespace oes

cd /home/vagrant
rm -rf PVDIR 2>&1 > /dev/null
mkdir -p PVDIR
...
echo "/vagrant/helm install oes . --namespace oes --timeout 10m"
/vagrant/helm install oes . --namespace oes --timeout 10m
