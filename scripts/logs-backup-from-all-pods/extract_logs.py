...
def get_no_of_active_users_count():
    if days.strip() != 'None':
        url = isd_gate_url + "/platformservice/v1/insights/widgetChartData?chartId=6&noOfDays=" + days
    else:
        url = isd_gate_url + "/platformservice/v1/insights/widgetChartData?chartId=6&noOfDays=180"
    cookie = login_to_isd()
    usage_charts = get_response(url, cookie)
    data_source = usage_charts['DataSource']
    data = data_source['data']
    active_users_count = 0
    for d in data:
        if d[0] == 'Active Users':
            active_users_count = active_users_count + d[2]
...
def start_extraction():
    try:
        ...
        f.write("no_of_users_executed_pipelines: "+ str(no_of_users_executed_pipelines)+ "\n")
        if installation_type == 'ISD':
            no_of_active_users_count = get_no_of_active_users_count()
            f.write("no_of_active_users_count: "+ str(no_of_active_users_count) + "\n")
