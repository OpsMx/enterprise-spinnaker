...
def create_audit_events():
    try:
        audit_cursor.execute(
            "CREATE TABLE IF NOT EXISTS audit_events (id serial PRIMARY KEY, created_at TIMESTAMPTZ, updated_at TIMESTAMPTZ, data jsonb, event_id VARCHAR(255) NOT NULL, source VARCHAR(50))")
    except Exception as e:
            ...
            for opsmx_id in result:
                try:
                    opsmxdb_cursor.execute(
                        "update userservicetemplate set verification_type = 'VERIFICATION' where opsmx_id=" + str(
                            opsmx_id))
                except Exception as e:
            ...
            for data in result:
                try:
                    ...
                    platform_cursor.execute(
                        'update datasource SET config =' + updatedConfig + ' where id =' + str(data[0]))
                except Exception as e:
