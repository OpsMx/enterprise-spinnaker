...
def perform_Audit_migration():
    try:
        ...
        url = verifySpinnakerConfigurationAndGetURL()
            ...
            for application, pipelineExecutions in applicationPipelines.items():
                for pipelineExecution in pipelineExecutions:
                    ...
                    if 'id' in pipelineExecution and 'buildTime' in pipelineExecution and 'status' in pipelineExecution:
                        ...
                        print("********** Received Rejected count of Pipeline Execution count::" + str(rejectedCount) + " application : " + str(application))
...
def getEventId(pipelineExecution):
    ...
    elif ('parentExecution' in pipelineExecution['trigger'] and 'trigger' in  pipelineExecution['trigger']['parentExecution'] and 'eventId' in pipelineExecution['trigger']['parentExecution']['trigger']):
            ...
            for application, pipelineConfigExecutions in applicationPipelines.items():
                for pipelineExecutionConfig in pipelineConfigExecutions:
                    ...
                    if 'name' in pipelineExecutionConfig and 'lastModifiedBy' in pipelineExecutionConfig and 'application' in pipelineExecutionConfig and 'id' in pipelineExecutionConfig and 'updateTs' in pipelineExecutionConfig:
                        ...
                        print("** Extracted pipeline config Data  user: " + user + ",application: " + application + ",pipelineId: " + pipelineId + ",pipelineName: " + pipelineName)
                        updated_pipeline_config_upper_json =  json.loads(pipeline_config_upper_json.replace('{user}',user).replace('{appName}', application).replace('{pipelineId}', pipelineId).replace('{pipelineName}', pipelineName))
                        updated_pipeline_config_execution_Json = json.dumps(updated_pipeline_config_upper_json)
                        print("Updated Pipeline config execution details for application : " + application + " :: " + updated_pipeline_config_execution_Json)
                        eventId = uuid.uuid4()
                        ...
                        print("********** Received Rejected count of Pipeline config Execution count::" + str(rejectedCount) + " application : "+str(application))
...
def insertPipelineConfigExecutionData(updatedTime,eventId, updated_pipeline_config_execution):
    try:
        ...
        data = str(updatedTime_date_time), str(updatedTime_date_time), updated_pipeline_config_execution, str(eventId), 'spinnaker '
        cur.execute(
            "INSERT INTO audit_events (created_at, updated_at,data,event_id,source) VALUES (%s, %s, %s, %s, %s)",
            data)
...
def migrate_policy_audit(policy_audits):
    try:
        ...
        for policy_audit in policy_audits:
            ...
            cur.execute("INSERT INTO audit_events (created_at, updated_at, data, event_id, source) VALUES (%s, %s, %s, %s, %s)", data)
...
def fetch_policy_audit():
    try:
        cur = oesdb_conn.cursor()
        cur.execute("select action, application, description, execution_id, gate, pipeline, policy_event, policy_name, result, user_id,created_date from policy_audit")
        return cur.fetchall()
    except Exception as e:
...
if __name__ == '__main__':
    n = len(sys.argv)
    if n != 12:
        print('Please pass valid 11 arguments visibilitydb <visibility-db-host> <platform_db-name> <platform_hostt> <opsmx-db-name> <opsmx-db-host> '
              '<oes-db-name> <oes-db-host> <audit_db-name> <audit_db_host> <db-port> <session_id>(configured spinnaker active session Id)')
